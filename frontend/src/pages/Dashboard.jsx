...
const CHART_COLORS = ['#f97316', '#fb923c', '#fdba74', '#ea580c', '#f59e0b', '#fb7185', '#facc15'];
  ...
  return (
    <div style={{ display: 'grid', gap: 22 }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 18 }}>
        <StatCard title="Base total" value={stats.total} helper="todos os itens controlados" accent="#f97316" icon="INV" />
        <StatCard title="Equipamentos ativos" value={stats.ativos} helper="itens prontos para operacao" accent="#fb923c" icon="OK" />
        ...
        <StatCard title="Com ticket" value={stats.com_ticket} helper="equipamentos vinculados a chamados" accent="#fdba74" icon="TK" />
      </section>
          ...
          {setorData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={setorData} margin={{ top: 10, right: 12, left: -18, bottom: 8 }}>
                ...
                <Tooltip cursor={{ fill: 'rgba(249,115,22,.08)' }} contentStyle={{ borderRadius: 14, border: '1px solid rgba(249,115,22,.16)', background: '#111', color: '#fff' }} />
                <Bar dataKey="value" radius={[10, 10, 0, 0]} fill="#f97316" name="Equipamentos" />
              </BarChart>
            </ResponsiveContainer>
            ...
            <>
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  ...
                  <Tooltip contentStyle={{ borderRadius: 14, border: '1px solid rgba(249,115,22,.16)', background: '#111' }} />
                </PieChart>
              </ResponsiveContainer>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 20 }}>
        <div style={{ ...cardStyle, padding: 24 }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            <InsightItem label="Em manutencao" value={`${stats.em_manutencao} item(ns)`} tone={{ background: 'rgba(249,115,22,.1)', border: 'rgba(249,115,22,.16)', labelColor: '#fdba74', valueColor: '#fff' }} />
